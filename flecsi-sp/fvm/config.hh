...
struct config_base : flecsi::topo::help {
  ...
  struct detail {
     ...
     */
    template<auto H, class Before, class After>
    struct connect_full;
    ...
    struct connect_full<H, has<>, has<AfterHead, AfterTail...>> {
      using type = typename list_prepend<from<H, to<AfterHead, AfterTail...>>,
        typename connect_full<AfterHead, has<H>, has<AfterTail...>>::type>::
        value;
    };
    ...
    struct connect_full<H, has<Before...>, has<AfterHead, AfterTail...>> {
      using type =
        typename list_prepend<from<H, to<Before..., AfterHead, AfterTail...>>,
          typename connect_full<AfterHead,
            has<Before..., H>,
            has<AfterTail...>>::type>::value;
    };
  ...
  struct fully_connected<has<H, V...>> {
    using type =
      typename detail::template connect_full<H, has<>, has<V...>>::type;
  };
};
