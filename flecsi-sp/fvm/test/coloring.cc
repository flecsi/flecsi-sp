...
gather_raw(std::vector<std::size_t> & raw)
{
    ...
    std::vector<std::size_t> full_raw(displs.get()[size-1] + rcounts.get()[size-1]);
    const auto typ = util::mpi::type<std::size_t>();
    MPI_Gatherv(raw.data(), raw.size(), typ,
                full_raw.data(), rcounts.get(), displs.get(),
                typ, 0, MPI_COMM_WORLD);
...
compute_coloring() {
  UNIT {
    ...
    auto [naive, ge, c2v, v2c, c2c] = topo::unstructured_impl::make_dcrs(def, 1);
    ...
    // auto [primaries, p2m, m2p] = topo::unstructured_impl::migrate(naive, colors, raw, c2v, v2c, c2c);
    // topo::unstructured_impl::coloring_definition cd{colors, 0, 2, 1, {{1, 0}}};
    // auto colorings = topo::unstructured_impl::color(def, cd, raw, primaries, c2v, v2c, c2c, m2p, p2m);
  };
}
int
coloring_driver() {
