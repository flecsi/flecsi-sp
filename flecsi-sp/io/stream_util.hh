...
template<class size>
struct block_cursor {
  block_cursor() : curr_block(0), entities_left(0),
  ...
  size current() { return curr_block; }
...
struct entity_cursor {
  entity_cursor() :
    base(0), block_base(0), new_block(true) {}
  ...
  size next_in_block() const {
    return (new_block or (curr.rowptr.size() == 0)) ? 0 : block_base + curr.rowptr.size() - 1;
  }
  ...
  void get(size id, std::vector<size> & ret) {
    if (contains(id)) {
      ...
      ret.resize(end - beg);
      std::copy(curr.colind.begin() + beg,
                curr.colind.begin() + end, ret.begin());
    }
  }
