...
static int
check_cell(std::size_t cellid,
  std::map<std::size_t, std::vector<std::size_t>> bless,
  const std::vector<std::size_t> & p2m,
  const crs & c2e,
  const crs & e2v,
  const std::map<std::vector<std::size_t>, std::size_t> & v2e) {
  UNIT {
    bool owns_cell = false;
    for(std::size_t row{0}; row < c2e.offsets.size() - 1; row++) {
      if(row < p2m.size() and p2m[row] == cellid) {
        owns_cell = true;
        std::vector<std::size_t> edges;
        for(std::size_t off{c2e.offsets[row]}; off < c2e.offsets[row + 1];
            off++) {
          edges.push_back(c2e.indices[off]);
        }
        std::vector<std::size_t> bless_edges;
        for(const auto & [key, _] : bless)
          bless_edges.push_back(key);
    ...
    if(owns_cell) {
      ...
      for(std::size_t row{0}; row < e2v.offsets.size() - 1; row++) {
        ...
        if(edge != bless.end()) {
          ...
          for(std::size_t off{e2v.offsets[row]}; off < e2v.offsets[row + 1];
              off++) {
  ...
  UNIT {
    ...
    // mixed
    {
      ...
      EXPECT_EQ(check_cell(166,
                  {{39, {212, 213}}, {40, {213, 214}}, {41, {212, 214}}},
                  p2m,
                  c2e,
                  e2v,
                  v2e),
